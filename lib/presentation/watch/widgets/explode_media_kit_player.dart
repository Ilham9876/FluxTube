...
/// MediaKit-based video player for Explode service
class ExplodeMediaKitPlayer extends StatefulWidget {
  const ExplodeMediaKitPlayer({
    super.key,
  ...
  @override
  State<ExplodeMediaKitPlayer> createState() => _ExplodeMediaKitPlayerState();
}

class _ExplodeMediaKitPlayerState extends State<ExplodeMediaKitPlayer> {
  ...
  @override
  void didUpdateWidget(covariant ExplodeMediaKitPlayer oldWidget) {
  ...
  void _restoreFromPipSync() {
    ...
    _availableQualities = widget.availableVideoTracks
        .where((v) => v.quality.isNotEmpty && v.url.isNotEmpty)
    ...
    debugPrint('[ExplodePlayer] Restored from PiP successfully (sync)');
  }
  ...
  Future<void> _initializePlayback() async {
    try {
      ...
      // Build available qualities list from muxed streams
      _availableQualities = widget.availableVideoTracks
          .where((v) => v.quality.isNotEmpty && v.url.isNotEmpty)
          .map((v) => GenericQualityInfo(
                label: v.quality,
  ...
  Future<void> _setupMediaSource(String quality) async {
    try {
      String? videoUrl;
      final bool isLive = widget.liveUrl != null;

      // For live streams, use the live URL (HLS)
      if (isLive) {
  ...
  Future<void> changeQuality(String newQualityLabel) async {
    ...
    try {
      ...
      if (currentPosition.inSeconds > 0 && widget.liveUrl == null) {
  ...
  double _getAspectRatio() {
    // Try to parse resolution from first available track
    if (widget.availableVideoTracks.isNotEmpty) {
      final resolution = widget.availableVideoTracks.first.resolution;
      // Resolution format is "widthxheight"
